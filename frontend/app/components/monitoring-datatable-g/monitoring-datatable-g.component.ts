...
export class MonitoringDatatableGComponent implements OnInit {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes['rows'] && this.rows && this.rows.length > 0 && this.colsname && Object.keys(this.colsname).length > 0 ) {
      this.columns = this._dataTableService.colsTable(this.colsname, this.rows[0]);
    }

    if (changes['colsname'] && this.colsname && Object.keys(this.colsname).length > 0 ) {
      this.filters = {};
    }
